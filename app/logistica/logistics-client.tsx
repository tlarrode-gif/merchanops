...
function Panel({ state }: { state: LogisticsState }) {
  ...
  return <div className="space-y-4"><div className="grid gap-3 md:grid-cols-4">{rows.map(([label, value, href]) => <Link key={label} href={String(href)} className="rounded-2xl border bg-white p-4 shadow-sm hover:border-slate-400"><p className="text-sm text-slate-500">{label}</p><p className="text-3xl font-bold">{value}</p></Link>)}</div><Card title="Alertas logísticas">{alerts.length ? alerts.map(a => <Link key={`${a.href}-${a.text}`} href={a.href} className="flex items-center justify-between border-t py-3 text-sm"><span><Badge tone={a.level} /> {a.text}</span><ArrowRight className="h-4 w-4" /></Link>) : <Empty text="Sin alertas activas." />}</Card><Card title="Últimas solicitudes">{state.requests.slice(0, 5).map(r => <Link key={r.id} href={`/logistica/solicitudes?id=${r.id}`} className="block border-t py-3 text-sm"><b>{r.code}</b> · {r.source_type}<p className="text-slate-500">{r.status} · {r.installer_name || "Sin instalador"} · {r.delivery_address || "Sin dirección"}</p></Link>)}{!state.requests.length && <Empty text="Aún no hay solicitudes logísticas." />}</Card></div>;
}
...
function Solicitudes({ state, q, detailId, commit }: ViewProps) {
  ...
  return (
    <Layout
      list={
        <Card title="Peticiones de material" action={<Link href="/?tab=servicios" className="rounded-xl border px-3 py-2 text-sm font-semibold">Crear desde Servicios</Link>}>
        ...
        <Detail title="Detalle petición" selected={selected}>
          {selected && (
            <div className="space-y-3">
              <Read label="Origen" value={<Link href={sourceHref(state.requirements.find(x => x.request_id === selected.id) || { source_type: selected.source_type, source_id: selected.source_id } as any)}>Abrir origen -&gt;</Link>} />
...
function Trace({ state, vin }: { state: LogisticsState; vin?: string | null }) { if (!vin) return null; const picking = state.pickings.find(p => p.lineas.some(l => l.vin_id === vin)); const envio = picking?.envio_id ? state.shipments.find(x => x.id === picking.envio_id) : null; const incident = state.incidents.find(x => x.vin_id === vin && !["resuelta", "cancelada"].includes(x.estado)); const pending = state.pendings.find(x => x.vin_id === vin && !["cerrado", "recibido"].includes(x.estado)); return <Card title="Trazabilidad VIN"><Read label="VIN" value={vin} /><Read label="Campaña" value={picking?.campana_id || incident?.campana_id || "Sin campaña"} /><Read label="Picking" value={picking ? <Link href={`/logistica/picking?id=${picking.id}`}>{picking.codigo}</Link> : "Sin picking"} /><Read label="Envío" value={envio ? <Link href={`/logistica/envios?id=${envio.id}`}>{envio.tracking || envio.estado}</Link> : "Sin envío"} /><Read label="Incidencia activa" value={incident ? <Link href={`/logistica/incidencias?id=${incident.id}`}>{incident.codigo}</Link> : "Sin incidencia"} /><Read label="Pendiente llegada" value={pending ? <Link href={`/logistica/pendientes?id=${pending.id}`}>Ver pendiente</Link> : "Sin pendiente"} /><Movements state={state} rows={state.movements.filter(x => x.vin_id === vin)} /></Card>; }
