        ...
        <table className="w-full table-fixed text-sm">
          <thead>
            <tr className="bg-slate-900 text-white">
              <th className="w-[22%] p-3 text-left">VIN / Farmacia</th>
              <th className="w-[12%] p-3 text-left">Semana</th>
              <th className="w-[9%] p-3 text-left">Provincia</th>
              <th className="w-[12%] p-3 text-left">Teléfono</th>
              <th className="w-[13%] p-3 text-left">Estado llamada</th>
              <th className="w-[15%] p-3 text-left">Último contacto</th>
              <th className="w-[8%] p-3 text-left">Alerta</th>
              <th className="w-[9%] p-3 text-left">Acción</th>
            </tr>
          </thead>
    ...
    <div className="fixed inset-0 z-50 bg-black/30">
      <aside className="ml-auto flex h-full w-full max-w-3xl flex-col bg-white shadow-2xl md:rounded-l-3xl">
        <div className="flex items-start justify-between gap-3 border-b p-4">
          ...
          <button onClick={onClose} aria-label="Cerrar ficha de llamada" className="rounded-xl border p-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-slate-900">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="flex-1 space-y-5 overflow-auto p-4">
          <Section title="Contexto">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Read label="Alto vinilo" value={formatMeasure(call.height)} />
              <Read label="Ancho vinilo" value={formatMeasure(call.width)} />
          ...
          <Section title="Acciones rápidas">
            <div className="flex flex-wrap gap-2">
              {(["Confirmado", "No contesta", "Incidencia en llamada", "Pospuesto en llamada", "Cancelado en llamada", "Requiere revisión operaciones"] as IsdinCallStatus[]).map(status => (
                <button key={status} onClick={() => quick(status)} className="rounded-2xl border bg-white px-3 py-2 text-sm font-medium focus-visible:outline focus-visible:outline-2 focus-visible:outline-slate-900">
                  {status}
                </button>
              ))}
            </div>
          ...
          <Section title="Actuación logística">
            <label className="flex items-center gap-2 rounded-2xl border bg-slate-50 p-3 text-sm">
              <input type="checkbox" checked={!!draft.requires_logistics_action} onChange={event => patch({ requires_logistics_action: event.target.checked })} />
              Requiere actuación logística
            </label>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <Select label="Tipo de necesidad" value={draft.logistics_need_type || ""} onChange={value => patch({ logistics_need_type: value })} options={["", "cambio_medidas", "reposicion", "material_danado", "material_incorrecto", "falta_material", "revision_logistica"]} />
              <Input label="Fecha necesaria" type="date" value={dateOnly(draft.logistics_required_date)} onChange={value => patch({ logistics_required_date: value })} />
            </div>
        ...
        <div className="flex flex-col gap-2 border-t p-4 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap gap-2">
            <a href={`/grandes-campanas/isdin?q=${encodeURIComponent(call.vin)}`} className="rounded-2xl border bg-white px-3 py-2 text-sm">
              <ExternalLink className="mr-1 inline h-4 w-4" />
              Ver vinilo relacionado
            </a>
            <button onClick={onCopySummary} className="rounded-2xl border bg-white px-3 py-2 text-sm">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button disabled={saving} onClick={() => save(true)} className="rounded-2xl bg-slate-900 px-4 py-2 text-sm text-white disabled:opacity-50">
...
function CallRow({ call, onOpen, onQuickStatus, onCopySummary }: { call: IsdinCall; onOpen: (call: IsdinCall) => void; onQuickStatus: (call: IsdinCall, status: IsdinCallStatus) => Promise<void>; onCopySummary: (call: IsdinCall) => Promise<void> }) {
  ...
  return (
    <tr className={alert ? "border-t bg-rose-50/60 align-top" : "border-t bg-white align-top"}>
      ...
      <td className="p-3"><PhoneCell phone={call.phone_number} /></td>
  ...
  return (
    <div className={callNeedsOperationsAlert(call) ? "rounded-2xl border border-rose-200 bg-rose-50 p-4" : "rounded-2xl border bg-white p-4"}>
      ...
      <div className="mt-3 grid gap-2 text-sm">
        ...
        <Read label="Teléfono" value={call.phone_number || "Sin teléfono"} />
...
function RowActions({ call, onOpen, onQuickStatus, onCopySummary }: { call: IsdinCall; onOpen: (call: IsdinCall) => void; onQuickStatus: (call: IsdinCall, status: IsdinCallStatus) => Promise<void>; onCopySummary: (call: IsdinCall) => Promise<void> }) {
  ...
  return (
    <div className="relative flex items-center gap-2">
      ...
      <button aria-label={`Más acciones para ${call.vin}`} onClick={() => setOpen(!open)} className="rounded-xl border bg-white p-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-slate-900">
...
function VinCell({ call }: { call: IsdinCall }) {
  return (
    <div>
      <p className="font-mono text-xs text-slate-500">{call.vin}</p>
      <p className="font-semibold leading-tight">{call.pharmacy_name}</p>
      <p className="text-xs text-slate-500">{call.city || "Sin ciudad"}</p>
    </div>
  );
}
...
function LastContact({ call }: { call: IsdinCall }) {
  return (
    <div className="text-xs">
      <p>{call.call_datetime ? String(call.call_datetime).replace("T", " ").slice(0, 16) : "Sin contacto"}</p>
      {call.contact_person && <p className="text-slate-500">{call.contact_person}</p>}
    </div>
  );
}

function PhoneCell({ phone }: { phone?: string | null }) {
  if (!phone) return <span className="text-xs text-slate-400">Sin teléfono</span>;
  return <a href={`tel:${phone}`} className="text-sm font-semibold text-slate-900 underline-offset-2 hover:underline">{phone}</a>;
}
...
function CallStatusBadge({ call }: { call: IsdinCall }) {
  return (
    <div>
      <span className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ring-1 ${callStatusClass(call.call_status)}`}>{cleanCallStatus(call.call_status)}</span>
      <p className="mt-1 text-[11px] text-slate-500">{callStatusGroup(call.call_status, call.requires_operations_review)}</p>
    </div>
  );
}
        ...
        {rows.slice(0, 10).map(row => (
          <div key={row.name}>
            ...
            <div className="mt-1 h-2 overflow-hidden rounded-full bg-slate-100">
              <div className="h-full bg-slate-900" style={{ width: `${Math.max(4, total ? (row.total / total) * 100 : 0)}%` }} />
            </div>
          </div>
        ))}
...
function Input({ label, value, onChange, type = "text" }: { label: string; value?: string | number | null; onChange: (value: string) => void; type?: string }) {
  return (
    <label className="block">
      ...
      <input type={type} value={value ?? ""} onChange={event => onChange(event.target.value)} className="w-full rounded-2xl border px-3 py-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-slate-900" />
    </label>
  );
}
...
function Textarea({ label, value, onChange }: { label: string; value?: string | null; onChange: (value: string) => void }) {
  return (
    <label className="mt-3 block">
      <span className="text-sm font-medium">{label}</span>
      <textarea value={value ?? ""} onChange={event => onChange(event.target.value)} rows={5} className="w-full rounded-2xl border p-3 focus-visible:outline focus-visible:outline-2 focus-visible:outline-slate-900" />
    </label>
  );
}
...
function Select({ label, value, onChange, options }: { label: string; value?: string | null; onChange: (value: string) => void; options: readonly string[] }) {
  return (
    <label className="block">
      <span className="text-sm font-medium">{label}</span>
      <select value={value ?? ""} onChange={event => onChange(event.target.value)} className="w-full rounded-2xl border bg-white px-3 py-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-slate-900">
