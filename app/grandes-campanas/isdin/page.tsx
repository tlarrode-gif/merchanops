...
type ColKey = "pharmacy_name" | "vinyl" | "status" | "comments" | "vinyl_record_type" | "vinyl_campaign" | "height" | "width" | "base_payment" | "payment_total" | "desired_installation_week" | "desired_installation_date" | "next_visit_date" | "street" | "street_number" | "city" | "postal_code" | "province" | "installer";
...
const localKey = "merchanops_isdin_local_v372";
...
const columns: ColumnDef[] = [
  ...
  { key: "payment_total", label: "Total", exportLabel: "PAGO TOTAL", compact: true },
  { key: "desired_installation_week", label: "Semana", exportLabel: "Fecha instalación deseada" },
  { key: "desired_installation_date", label: "Fecha", exportLabel: "Fecha deseada", compact: true },
...
function parseRows(text: string, workers: Worker[]): IsdinVinyl[] {
  return text.split("\n").map(x => x.trim()).filter(Boolean).filter(line => !line.toLowerCase().startsWith("nombre farmacia;")).map(line => {
    ...
    const item: IsdinVinyl = {
      id: uid(),
      pharmacy_name: clean(pharmacy_name),
      vinyl: clean(vinyl),
      status,
      comments: clean(comments),
      vinyl_record_type: clean(vinyl_record_type),
      vinyl_campaign: clean(vinyl_campaign),
      height: n(height),
      width: n(width),
      base_payment: n(base_payment),
      failed_visit_payment: FAILED_VISIT,
      desired_installation_week: clean(desired_installation_week),
      street: clean(street),
      ...
      installer_name: installer?.name || clean(installerRaw) || null,
      ceco: CECO,
      client: CLIENT,
      payment_week: clean(desired_installation_week),
      incident_opened_at: status === "Incidencia" ? nowISO() : null,
...
function colValue(x: IsdinVinyl, key: ColKey) { if (key === "installer") return x.installer_name || ""; if (key === "base_payment") return eur(Number(x.base_payment || 0)); if (key === "payment_total") return eur(Number(x.payment_total || 0)); return String((x as any)[key] ?? ""); }
function rawSortValue(x: IsdinVinyl, key: ColKey | "") {
  if (!key) return "";
  if (key === "installer") return x.installer_name || "";
  if (["height", "width", "base_payment", "payment_total"].includes(key)) return Number((x as any)[key] || 0);
  return String((x as any)[key] ?? "").toLowerCase();
}
...
export default function IsdinPage() {
  ...
  async function updateItem(item: IsdinVinyl, patch: Partial<IsdinVinyl>) {
    ...
    if (patch.status === "Incidencia llamada") next.incident_resolved_at = null;
  ...
  const filtered = items.filter(x => {
    const hay = columns.map(c => colValue(x, c.key)).concat([x.payment_week || "", x.ceco || ""]).join(" ").toLowerCase();
    const week = String(x.payment_week || x.desired_installation_week || "");
    return (!filters.q || hay.includes(filters.q.toLowerCase())) && (!selectedStatuses.length || selectedStatuses.includes(String(x.status || "Nuevo"))) && (!filters.installer || x.installer_id === filters.installer) && (!filters.province || x.province === filters.province) && (!selectedWeeks.length || selectedWeeks.includes(week));
  });
  const sortedFiltered = [...filtered].sort((a, b) => {
    if (!sort.key) return 0;
    const av = rawSortValue(a, sort.key);
    const bv = rawSortValue(b, sort.key);
    const res = typeof av === "number" && typeof bv === "number" ? av - bv : String(av).localeCompare(String(bv), "es", { numeric: true, sensitivity: "base" });
    return sort.direction === "asc" ? res : -res;
  });
  const workerKpis = useMemo(() => workers.map(w => { const own = sortedFiltered.filter(x => x.installer_id === w.id || (!x.installer_id && x.installer_name === w.name)); return { name: w.name, total: own.length, finalizados: own.filter(x => x.status === "Finalizado").length, incidencias: own.filter(x => x.status === "Incidencia").length, bloqueos: own.filter(x => x.status === "Incidencia llamada").length, pendientes: own.filter(x => x.status === "Nuevo" || x.status === "Resuelto - Pendiente colocador" || x.status === "Incidencia llamada").length, pago: own.reduce((a, x) => a + Number(x.payment_total || 0), 0), tasaFinalizado: pct(own.filter(x => x.status === "Finalizado").length, own.length), tasaIncidencia: pct(own.filter(x => x.status === "Incidencia").length, own.length) }; }).filter(x => x.total > 0).sort((a,b)=>b.total-a.total), [workers, sortedFiltered]);
  const stats = useMemo(() => { const total = sortedFiltered.length; const finalizados = sortedFiltered.filter(x => x.status === "Finalizado").length; const cancelados = sortedFiltered.filter(x => x.status === "Cancelado").length; const incidencias = sortedFiltered.filter(x => x.status === "Incidencia").length; const incidenciaLlamada = sortedFiltered.filter(x => x.status === "Incidencia llamada").length; const pendientesColocador = sortedFiltered.filter(x => x.status === "Resuelto - Pendiente colocador").length; const nuevos = sortedFiltered.filter(x => x.status === "Nuevo").length; const asignados = sortedFiltered.filter(x => !!x.installer_id || !!x.installer_name).length; const pagos = sortedFiltered.reduce((a, x) => a + Number(x.payment_total || 0), 0); const fallidas = sortedFiltered.filter(x => ["Incidencia", "Cancelado", "Resuelto - Pendiente colocador"].includes(String(x.status))).length; return { total, finalizados, cancelados, incidencias, incidenciaLlamada, pendientesColocador, nuevos, asignados, pagos, fallidas, avance: pct(finalizados + cancelados, total), pendiente: total - finalizados - cancelados, tasaIncidencia: pct(incidencias, total), tasaFallida: pct(fallidas, total), sinInstalador: total - asignados, ahorroPreventivo: incidenciaLlamada * FAILED_VISIT }; }, [sortedFiltered]);
...
function StatusLine({label,value,total}:any){const p=pct(value,total);return <div className="mb-2"><div className="flex justify-between text-sm"><span>{label}</span><b>{value} · {p}%</b></div><div className="h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-slate-900" style={{width:`${Math.max(3,p)}%`}}/></div></div>}
