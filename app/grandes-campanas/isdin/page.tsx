...
const localKey = "merchanops_isdin_local_v1";
...
function workerByExactName(workers: Worker[], name?: string | null) {
  const target = clean(name);
  if (!target) return null;
  return workers.find(w => clean(w.name) === target) || null;
}
...
function parseRows(text: string, workers: Worker[]) {
  const lines = text.split("\n").map(x => x.trim()).filter(Boolean);
  const rows = lines.filter(line => !line.toLowerCase().startsWith("nombre farmacia;")).map(line => {
    ...
    item.payment_total = calcPayment(item);
    item.payment_ready = paymentReady(item);
    return item;
  });
  return rows.filter(r => r.pharmacy_name || r.vinyl);
}
...
export default function IsdinPage() {
  ...
  function recalc(v: IsdinVinyl) {
    const payment_total = calcPayment(v);
    const payment_ready = paymentReady(v);
    return { ...v, payment_total, payment_ready, ceco: CECO, client: CLIENT };
  }

  async function importRows() {
    const rows = parseRows(paste, workers);
    if (!rows.length) { saved("No hay filas válidas"); return; }
    const ready = rows.map(recalc);
    if (isSupabaseConfigured && supabase) {
    ...
    setShowImport(false);
    saved(`${ready.length} vinilos importados`);
  }

  async function updateItem(item: IsdinVinyl, patch: Partial<IsdinVinyl>) {
    let next = { ...item, ...patch, status_changed_at: nowISO() };
    ...
    if (patch.next_visit_date) {
      const week = weekLabelFromDate(patch.next_visit_date);
      next.desired_installation_date = patch.next_visit_date;
      next.desired_installation_week = week;
      next.payment_week = week;
    }
    if (patch.desired_installation_date) {
      const week = weekLabelFromDate(patch.desired_installation_date);
      next.desired_installation_week = week;
      next.payment_week = week;
    }
    if (patch.installer_id !== undefined) {
      const w = workers.find(x => x.id === patch.installer_id);
      next.installer_name = w?.name || null;
    }
    ...
    if (isSupabaseConfigured && supabase) {
      const { id, ...db } = next;
      await supabase.from("isdin_vinyls").update(db).eq("id", item.id);
    }
    saved();
  }

  async function deleteItem(item: IsdinVinyl) {
    if (!confirm(`¿Borrar ${item.vinyl}?`)) return;
    setItems(prev => prev.filter(x => x.id !== item.id));
    if (isSupabaseConfigured && supabase) await supabase.from("isdin_vinyls").delete().eq("id", item.id);
    saved("Vinilo borrado");
  }
  ...
  const filtered = items.filter(x => {
    const hay = [x.pharmacy_name, x.vinyl, x.status, x.comments, x.vinyl_record_type, x.vinyl_campaign, x.desired_installation_week, x.street, x.street_number, x.city, x.postal_code, x.province, x.installer_name, x.payment_week].join(" ").toLowerCase();
    return (!filters.q || hay.includes(filters.q.toLowerCase())) && (!filters.status || x.status === filters.status) && (!filters.installer || x.installer_id === filters.installer) && (!filters.province || x.province === filters.province) && (!filters.week || (x.payment_week || x.desired_installation_week) === filters.week);
  });

  const stats = useMemo(() => ({
    total: filtered.length,
    ready: filtered.filter(paymentReady).length,
    incidents: filtered.filter(x => x.status === "Incidencia").length,
    pending: filtered.filter(x => x.status === "Nuevo").length,
    payments: filtered.reduce((a, x) => a + Number(x.payment_total || 0), 0)
  }), [filtered]);
  ...
  function exportPayments() {
    const rows = filtered.filter(paymentReady).map(x => [CLIENT, CECO, x.payment_week || x.desired_installation_week || "Sin semana", x.installer_name || "Sin instalador", x.pharmacy_name, x.vinyl, x.status, x.payment_total || 0]);
    downloadCSV("pagos_isdin.csv", [["Cliente", "CECO", "Campaña/Semana", "Trabajador", "Farmacia", "VIN", "Estado", "Total"], ...rows]);
  }
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-900">
    <header className="sticky top-0 z-10 border-b bg-white">
      <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-4 md:flex-row md:items-center md:justify-between">
        <div>
          <a href="/grandes-campanas" className="mb-2 inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-900"><ArrowLeft className="h-4 w-4" /> Volver a Grandes Campañas</a>
          <h1 className="text-3xl font-bold">ISDIN · Vinilos</h1>
          <p className="text-sm text-slate-500">CECO fijo {CECO}. Gestión por VIN, semana, instalador, estado, incidencias y pagos.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setShowImport(v => !v)} className="rounded-2xl bg-slate-900 px-4 py-2 text-white"><Upload className="mr-1 inline h-4 w-4" />Carga masiva</button>
          <button onClick={exportAll} className="rounded-2xl border bg-white px-4 py-2"><FileDown className="mr-1 inline h-4 w-4" />Exportar datos</button>
          <button onClick={exportPayments} className="rounded-2xl border bg-white px-4 py-2"><FileDown className="mr-1 inline h-4 w-4" />Exportar pagos</button>
        </div>
      </div>
    </header>
