...
function weekLabelFromDate(date = new Date()) { const d = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 12); const oneJan = new Date(d.getFullYear(), 0, 1); const week = Math.ceil((((d.getTime() - oneJan.getTime()) / 86400000) + oneJan.getDay() + 1) / 7); const month = new Intl.DateTimeFormat("es-ES", { month: "long" }).format(d); return `Semana ${week} ${month.charAt(0).toUpperCase() + month.slice(1)} ${d.getFullYear()}`; }
function weekNumFromLabel(label?: string | null) { const m = String(label || "").match(/Semana\s+(\d+).*?(20\d{2})/i); return m ? { week: Number(m[1]), year: Number(m[2]) } : null; }
function currentWeekInfo() { const now = new Date(); const label = weekLabelFromDate(now); return { label, ...weekNumFromLabel(label)! }; }
function itemWeek(v: IsdinVinyl) { return v.payment_week || v.desired_installation_week || "Sin semana"; }
function isCurrentWeek(v: IsdinVinyl) { const cur = currentWeekInfo(); const parsed = weekNumFromLabel(itemWeek(v)); if (parsed) return parsed.week === cur.week && parsed.year === cur.year; const d = v.desired_installation_date ? new Date(v.desired_installation_date + "T12:00:00") : null; return !!d && weekLabelFromDate(d) === cur.label; }
function isPastWeek(v: IsdinVinyl) { const cur = currentWeekInfo(); const parsed = weekNumFromLabel(itemWeek(v)); if (parsed) return parsed.year < cur.year || (parsed.year === cur.year && parsed.week < cur.week); const d = v.desired_installation_date ? new Date(v.desired_installation_date + "T12:00:00") : null; return !!d && d.getTime() < Date.now(); }
