...
import { AppPermissionKey, AppSession, AppUser, adminPermissions, canAccessModule, defaultPermissions, ensureAuthSession, filterBySessionProvince, getCurrentAppSession, isAdminSession, loadInternalUsers, loginAppUser, normalizeUser, roleLabels, saveCurrentAppSession, saveInternalUsers, userCanSeeProvince } from "@/lib/access-control";
...
// Colores de calendario en la paleta de marca. Entran por estilo inline, así
// que el CSS no puede reencajarlos: tienen que nacer ya correctos.
const calendarColors:any={slate:{label:"Tinta",bg:"#14141f"},blue:{label:"Azul",bg:"#3355a8"},sky:{label:"Celeste",bg:"#1f6f8b"},violet:{label:"Violeta",bg:"#5c4880"},pink:{label:"Coral",bg:"#e94560"},orange:{label:"Oro",bg:"#b9aa83"},green:{label:"Verde",bg:"#15803d"},amber:{label:"Ámbar",bg:"#8a6a1f"},red:{label:"Burdeos",bg:"#8b1a2f"},teal:{label:"Turquesa",bg:"#2f5d3a"}};
// Nombres de módulo tal y como se llaman en la interfaz.
const moduleLabels:Record<string,string>={servicios:"Servicios",isdin:"ISDIN",calendario:"Calendario",pagos:"Pagos",logistica:"Logística",rrhh:"RR.HH.",usuarios:"Usuarios"};
  ...
  return (
    <div className="space-y-4">
      <MoKpiGrid>
        ...
        <MoKpi label="Cumplimiento del mes" value={`${compliance}%`} hint={`${onTimeCount} de ${validatedThisMonth.length} a tiempo · ${pct(validated.filter(onTime).length, validated.length)}% histórico`} accent="ok" icon={CheckCircle2} />
