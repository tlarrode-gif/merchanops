 ...
 */
export function puntoRowToEngineInput(punto: Row, incidencias: IncidenciaRow[] = []): CampanaPuntoInput {
...
export async function syncCampanaObligations(
  ...
  const inputs = puntos.map(punto => puntoRowToEngineInput(punto, incidencias)).filter(input => input.id);
